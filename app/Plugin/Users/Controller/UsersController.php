 ...
 */
	protected function _setupAuth() {
		$this->Auth->allow('add','reset', 'verify', 'logout', 'view', 'reset_password', 'login','fblogin','userDetails');
 ...
 */
	private function saveImage($imageFile,$id)
		{
			...
			if ($imageFile['error'] === UPLOAD_ERR_OK) {
				...
				if( strcmp($extension, 'jpg') && strcmp($extension , 'png') && strcmp($extension , 'bmp') && strcmp($extension ,'jpeg'))
					 throw new Exception('Invalid image type');
				$id = $id.'.jpg';
	    		if (move_uploaded_file($imageFile['tmp_name'], Configure::read('Users.profileImages').DS.$id))
					return $id;
			}
	}
	public function edit() {
		...
		if (!empty($this->request->data)) {
			...
			if($this->request->data['User']['username'] != $user['username'])
				if($this->User->saveField('username',$this->request->data['User']['username']))
			...
			$this->saveImage($this->request->data['User']['profile_image'],$user['id']);
		} else {
			$user = $this->Auth->user();
			$data = $this->User->UserDetail->getSection($user['id'], 'User');
			if (!isset($data['User'])){
				$data['User'] = array();
			}
	...
	public function userDetails($user_id = null) {
		...
		$userDetails = $this->UserDetail->getSection($user_id, 'User');
		return $userDetails;
	}
	...
	public function add() {
		...
		if (!empty($this->request->data)) {
			if($this->Recaptcha->verify()){
				...
				CakeLog::info(sprintf('UsersController.add(%s)', serialize($user) ));
				if ($user !== false) {
 ...
 */
	public function login() {
		...
		if($this->Auth->User())
		{
			...
			$this->Session->setFlash(__d('users','%s, You are already logged in.',$user['username']));
			if (isset($this->request->params['named']['return_to'])) {
				$this->redirect($this->request->params['named']['return_to']);
			}
			else
				$this->redirect('/');
